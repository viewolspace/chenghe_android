...
import com.baihe.base.base.App;


/**
 * @author : sklyand
 * @email : zhengdengyao@51yryc.com
 * @time : 2019/7/18 16:22
 * @describe ：
 */
public class ToastUtils {
    ...
    private static void showToast(@StringRes int resId, int duration) {
        showToast(App.getInstant().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void showToast(@StringRes int resId, int duration, Object... args) {
        showToast(String.format(App.getInstant().getResources().getString(resId), args), duration);
    }
    ...
    private static void showToast(CharSequence text, int duration) {
        ...
        if (sToast == null) {
            sToast = Toast.makeText(App.getInstant(), text, duration);
        } else {
            sToast.setText(text);

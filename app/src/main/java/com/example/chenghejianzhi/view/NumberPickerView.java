package com.example.chenghejianzhi.view;
...
public class NumberPickerView extends View {
    ...
    public void setTextSizeNormal(int mTextSizeNormal) {
        this.mTextSizeNormal = mTextSizeNormal;
    ...
    public int getTextSizeHint() {
        return mTextSizeHint;
    }

    public void setTextSizeHint(int mTextSizeHint) {
        this.mTextSizeHint = mTextSizeHint;
        postInvalidate();
    }
    ...
    private void drawContent(Canvas canvas) {
        ...
        for (int i = 0; i < mShowCount + 1; i++) {
            ...
            // TODO: 2017/7/24  渐变字号，颜色
            int[] colors = {Color.parseColor("#C6C6C6"), Color.parseColor("#999999"), Color.parseColor("#666666"),
                    Color.parseColor("#333333"),
                    Color.parseColor("#666666"), Color.parseColor("#999999"), Color.parseColor("#C6C6C6")};
            if (mShowCount == 7) {
                if (i == 0) {
                    textSize = dp2px(mContext,14);
                    textColor = colors[0];
                } else if (i == 1) {
                    textSize = dp2px(mContext,16);
                    textColor = colors[1];
                } else if (i == 2) {
                    textSize = dp2px(mContext,18);
                    textColor = colors[2];
                } else if (i == 3) {
                    textSize =dp2px(mContext,24);
                    textColor = colors[3];
                } else if (i == 4) {
                    textSize = dp2px(mContext,18);
                    textColor = colors[4];
    ...
    private void drawHint(Canvas canvas) {
        ...
        if (language.equals("ar") | language.equals("fa")) {
            canvas.drawText(mHintText,
                    ...
                    (dividerY0 + dividerY1) / 2 + mTextSizeHintCenterYOffset - 10, mPaintHint);
        } else {
            canvas.drawText(mHintText,
                    mViewCenterX + (mMaxWidthOfDisplayedValues + mWidthOfHintText) / 2 + mMarginStartOfHint,
                    (dividerY0 + dividerY1) / 2 + mTextSizeHintCenterYOffset - 5, mPaintHint);
        }
        // TODO: 2017/7/24 阿拉伯提示显示左面
    }
